...
export interface StandardParseResult {
   ...
   * under the symbol current at its export, so an OCC hint is rebased forward
   * from here. A converter reports what the file states and never a substitute --
   * the upload decides what to do about a format that states nothing, and a
   * guess made here would be indistinguishable from the file's own claim. See
