...
export function counterLeg(p: Posting): Posting | undefined {
  ...
  // A boundary leg is not a leg of the record: it is inferred from the declared
  // type alone, and the source neither wrote it nor named it. The clone above
  // would otherwise hand it the evidence of the posting it mirrors, which would
  // say the source correlated a row it never wrote. moneyLeg's legs are read out
  // of a record and do carry its correlations. See docs/spec/postings.md.
  leg.correlations = [];
  return leg;
}
