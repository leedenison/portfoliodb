...
for prices and corporate events, the **statement** for holding declarations.
Coverage, asset class and currency sit on the group; the file envelope carries
only `format_version`, `exported_at` and the source instance; rows carry only
what varies per row.

Transactions were on that list and are not any more. A tx group has no fields of
its own -- it carries neither an id nor a timestamp, both being generated by the
importing instance -- so once
adr/0041-server-owns-transaction-grouping.md made the partition derived there was
nothing left in the level to state. See
adr/0043-grouping-does-not-travel-in-the-archive.md. A level admitted for a field
that cannot vary below it is not admitted for no field at all.
