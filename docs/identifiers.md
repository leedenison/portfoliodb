...
Clients may also pass hints along with each transaction for `exchange_code`, `currency` and `mic`.  These can be used to narrow instrument resolution (see plugins below).  The hints must never be stored as canonical information directly.  They can only be used to narrow resolution with the authoritative data coming from the plugin resolution.

Clients may also pass known, external identifiers for a transaction (eg. `"ISIN"`, `"CUSIP"`, `"FIGI"`, `"TICKER"`, etc).  
