...
- The **system archive** carries shared data and no user data: instruments and
  ...
  inflation indices, fetch blocks and plugin configuration.
...
- **System:** instruments, then prices, then corporate events, then inflation
  indices, fetch blocks and plugin configuration. Prices, events and fetch blocks
  reference instruments. Inflation indices and plugin configuration reference
  nothing, and plugin configuration is written last because it is what makes a
  document a secret.

  Corporate events a cycle could not apply are **not** carried. They are
  telemetry (adr/0080), and telemetry is retained on a clock rather than
  archived. Most of them are re-derived by the corporate event cycle from the
  splits and identity the archive does carry; two kinds of dividend are not, and
  that gap is
