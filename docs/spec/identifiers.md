...
A security whose currency is unknown carries at most one listing with a null currency. That is a different thing from a security with exactly one listing, which has a currency and no siblings. **An unknown listing is not priceable and not event-bearing**: a price with no stated currency asserts nothing, so no price, coverage row, dividend or fetch block is written against one, and a holding on one reports unpriced. Cash and FX instruments have a listing degenerately -- a cash instrument's own currency, and an FX pair's quote currency, which is USD under adr/0006-fx-as-synthetic-instruments.md.

See adr/0068-a-listing-is-a-currency-of-a-security.md.
...
Consistency checks between identifier plugin results compare identifiers on the whole triple. Two results naming one ticker under different domains are naming different listings and are not in agreement, however equal the values are; they are in disagreement, and the lower-precedence result is excluded from the merge. An identifier naming no domain names no particular listing, and is not compared against one that does.
