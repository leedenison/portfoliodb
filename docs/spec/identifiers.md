...
**Grain** says what the values name: the security, or one listing of it. MIC_TICKER and OPENFIGI_TICKER name a listing, and the domain says which one; every other type names the security, and its domain -- the source that wrote a description, the broker that issued a contract number -- names something beside the value rather than scoping it. So the grain decides what a domain does. Two listing-scoped values under two named domains are two listings, which is why a symbol appearing under both is not two results agreeing; two security-scoped values under two domains are two names for one thing. A contract symbol is security-scoped: a contract is its own security, cleared in one place, however many venues its underlying trades on.
