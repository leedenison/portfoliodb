...
Each `identifier_type` carries four properties that the rules below key off. None of them says who vouched for a value: that is the authority of the source that supplied it, any type can arrive at either level, and it is recorded as the row's owner rather than declared per type.
...
The type property is necessary and never sufficient: an association the system holds as a **claim** mediates nothing whatever its type, because identifier rows are owner-scoped while instruments are not, so a chain drawn through one would merge instance-global rows on the strength of one unauthenticated file. A broker contract identifier is the case to watch -- its issuer may well never reassign it, and it still mediates nothing until the promotion sweep makes it system-owned (see **Ownership of user-supplied mappings** below).
...
What cannot hold is two values for one **subject** under one instrument description. The subject rather than the type, because a ticker under two domains names two listings and a security quoted in two places states both legitimately; the description because it is the key resolution caches on, so two values under one description are two securities where the file names one. The converse is not a contradiction and is not looked for: a broker may write one security several ways, and those descriptions resolve to one instrument, which is the point of storing the mapping. Domains are compared as the file spells them, so a segment MIC beside the operating MIC it normalises to is accepted -- this refuses a whole upload, so it fires only where the disagreement is plain in the file itself.

The check is wrapped around every registered converter rather than called by each one, so a converter added later cannot omit it. The server checks again because what makes an upload acceptable has to hold wherever it arrives from: the browser extension carries converters of its own, and the ingestion API is reachable without either.

If **the file and the database** disagree, this is an identity failure and not a transaction failure. The transactions were never in doubt; only which instrument they belong to is. The instrument degrades to broker-description-only, exactly as an identifier plugin timeout already causes, the contradiction is recorded, and the upload is accepted. Rejecting it would strand the user behind an admin for a corporate action neither knew about. The posting carries an identification error saying why it did not identify, and the disagreement itself -- which stated name reached which instrument -- is a `telemetry.conflicting_hint` row under the key. Nothing is asked of the identifier plugins first: the names that disagreed are exactly what would be put to them, so a provider would be asked to settle a question about this instance's own database, and whichever it answered would leave the other name pointing elsewhere.
...
If the client supplies only a broker, source and description with optional hints, the system will attempt to extract candidate identifiers from the description.  This is done via the "description" plugins at `server/plugins/<datasource>/description` (see below).  If the extraction succeeds then resolution continues using the extracted identifiers.  **A (source, NULL, description) identifier must be stored in this case as a mapping from the broker description, owned by the user whose upload carried it.**

Extraction failure is treated as an identity lookup failure.  **A (source, NULL, description) identifier must be stored in this case as a mapping from the broker description, owned by the user whose upload carried it.**
...
If no plugin resolves a given (source, instrument_description), the system ensures an instrument exists with at least that source identifier and attaches it to the transaction. Plugin failures (e.g. timeout, unavailable) are handled the same way: persist the transaction with the broker-description-only instrument and record an identification error; do not fail the whole job. **What the uploader is told** is feedback rather than diagnostics, and its vocabulary is what changes what they do next rather than which of our stages gave up. Three outcomes: the row was **not identified** and is tracked by the broker's description, which supplying an identifier changes; **identification was unavailable** because a provider did not answer, which the same file uploaded later may resolve with no change at all; or the row's **identifiers name different securities** in this instance, which nothing here can settle. A fourth would need a fourth answer to "and then what".

Which stage gave up -- nothing proposed from the description, a proposal no provider recognised, a result nothing corroborated, a timeout, an unavailable plugin -- is recorded in full on `telemetry.resolution_key`, under the run. That is the question a reader asks about this system rather than about their file, and it is answered where the other questions about this system are. See [telemetry.md](telemetry.md).

**Instrument merge**: A merge acts on an **identity claim** -- an assertion that two identifiers denote one instrument -- and never on anything else. The claim must be corroborated by an authority: one identifier plugin result naming both identifiers -- by returning both, or by returning one while strictly filtering on the other -- or a chain through a third identifier whose association is system-owned, whose type does not routinely reassign its values, and whose two halves have overlapping validity intervals. A value the result was **strictly filtered on** counts as named: a provider that answers "no identifier found" when its filter matches nothing has asserted that the filtered value denotes the security it described, so OpenFIGI mapping a stated ISIN and answering with a FIGI corroborates that pair even though it deliberately does not echo the ISIN back. A filter the provider silently relaxes is a hint and confirms nothing. A set of identifiers assembled by the resolver from several identifier plugin results is not a claim anybody made, and does not merge; two results agreeing about a currency and a venue have not said they are the same security, and treating that as identity is how two share classes on one venue become one instrument. Remaining fields are still checked for consistency, but only identity claims merge. That is the case where the claim carries **system authority**; what a claim carrying user authority may do instead is in **Two classes of instrument** below. **Where nothing corroborates the merge the transaction still resolves**: it attaches to the instrument the highest-precedence identifier reached -- the winning result's own answer -- and every other instrument the resolution's identifiers landed on is left exactly as it was, holding the names and the transactions it already had. A name the resolution supplied that reached no instrument at all is written on to the one it attached to, owned by whoever supplied it, which is what makes a second upload of the same file resolve the same way; a name another instrument holds is not, because carrying it across would fail the exclusion constraint. See adr/0060-an-identity-claim-is-admitted-by-the-authority-of-its-source.md and adr/0061-transitivity-needs-a-non-reassigned-identifier.md.
...
**Metadata is security-level or listing-level, and they attach to different rows.** Only an identifier plugin is authoritative for the metadata of a system-authoritative instrument, and metadata never triggers a merge. Security-level metadata -- name, asset class, CIK, SIC code, and a contract's terms: its strike, its expiry, the right it confers and the line its strike is quoted in -- is a fact about the security and propagates across every identifier corroborated as denoting it. The terms are denormalised from a name rather than supplied beside it, so they arrive carrying whatever authority named the contract, and a match fills the ones the instrument has no answer for rather than restating the ones it has. Listing-level metadata -- currency and venue -- is a fact about one listing, and is written on to the listing the result named. This used to need a rule forbidding it to propagate through a security-grain identifier, because there was one row and a currency learned from the London line would be asserted of the New York one. There is now a row per line, so the constraint is structural rather than a rule: a result that names a line supplies that line's currency and venue, and a result that names only the security supplies neither.

A result excluded from the merge as inconsistent records what it argued about on its plugin call row -- the subject, what each result said, and which result won -- so the disagreement accumulates rather than being re-logged on every upload. See [telemetry.md](telemetry.md).
...
### Authority, facts and claims

**Authority is the level a source carries, not a property of the identifier it supplies.** **System authority** is what an identifier plugin, reference data and an admin's archive carry: an authenticated channel that can be asked again. **User authority** is what a transaction upload carries -- a broker file, or the postings of a user archive -- which is unauthenticated, single-shot and impossible to re-interrogate. Which source within a level supplied a value decides nothing: every identifier plugin is equally authoritative for a global identifier.

Any identifier type can arrive at either level. An ISIN stated in a broker file and an ISIN an identifier plugin returned are the same value carrying different weight. The scope of an identifier -- a registry's namespace, one broker's numbering, one source's own text -- says what the value is good for and nothing about who vouched for it.

**A fact** is an association or a piece of metadata the system treats as settled: it arrived from a system-authoritative source, or a system-authoritative source corroborated it. **A claim** is one the system treats as possibly true and not yet settled: it arrived from a user-authoritative source and nothing has corroborated it. Facts are stored system-owned and claims are stored owned by the user whose file carried them, so the owner column is where the distinction lives.

### Two classes of instrument

**Every identifier and every piece of metadata on an instrument has been confirmed by a source authoritative for that instrument's class.** A **system-authoritative** instrument holds at least one identifier that arrived from a source carrying system authority. A **user-authoritative** instrument holds only identifiers and metadata that arrived from user-authoritative sources -- whatever named it, in the cases where nothing could corroborate the naming. A broker-description-only instrument is the case of this the system has always had, stated as a class rather than as a shape. The class is read from the owner column, never from the identifier's type.

**Metadata follows the identifiers.** Everything on a user-authoritative instrument arrived from a user-authoritative source, by definition, so the class of the instrument is the provenance of its metadata and nothing has to be recorded per column to know it. When such an instrument becomes system-authoritative, its own metadata is discarded and replaced by what the authority supplied, rather than merged with it or kept on the strength of having been stored first. That happens where an instrument holding no identity is completed in place; the survivor of a merge is always system-authoritative already, so there is no second place it can happen. Every column is governed by the rule, an option's contract terms included; the identifier those were read from is not, a name a user supplied being owned rather than discarded.

**Metadata is never owned by a user, where an identifier can be, and the asymmetry is deliberate.** One rule stands behind both -- keep a claim where keeping it is cheap and something could later settle it -- and metadata satisfies neither half. Identifiers live in a discrete space with a natural key, so a claim costs one row and two users either agree exactly or conflict, which is what the promotion sweep counts; two names differing by a suffix are neither agreement nor conflict, so no sweep could promote a metadata claim however many users held it. Promotion would buy nothing if it could, because its purpose is to let a fact mediate a merge and metadata never merges. And owning metadata would require a viewing user on every read of an instrument, which the price fetch and the corporate event cycle do not have. The one piece of user-supplied metadata that is really evidence -- the broker's own description -- is an identifier, and is owned under the identifier rule.

**What a merge does is decided by the authority of the claim asking for it, and by what acting on it would change.** A claim carrying system authority settles the link and the instruments merge outright. A claim carrying user authority may move only what is itself a claim: the user's own identifier rows are repointed onto the other instrument together with the postings that resolved through them, nothing on the target is rewritten and no row changes owner, and the instrument the rows left stands for its other owners. A claim carrying user authority that would have to move a fact is refused, because acting on it would settle an association on the strength of one unauthenticated file -- which is what the promotion sweep below does properly. A refusal is not permanent: a later plugin result and the sweep are two routes by which the claim becomes a fact.

See adr/0060-an-identity-claim-is-admitted-by-the-authority-of-its-source.md and adr/0079-an-instrument-carries-the-authority-of-the-source-that-named-it.md.

### Ownership of user-supplied mappings

A broker file reaches the system through a user. It is unauthenticated, so nothing attests it came from the account it names; it is a single artefact, so a stale export, a misgrab or a hand edit is invisible; and it cannot be re-interrogated. Instruments are instance-global, so a merge driven by one upload rewrites reference data for every user. Sometimes there is no second opinion to be had -- a broker is the only possible authority for its own contract identifiers and descriptions -- so the trust is forced, and what bounds it is ownership.

**A claim is owned where nothing could have settled it.** What a user-authoritative claim becomes is decided by the enabled identifier plugins rather than by the scope of the identifier, and there are four answers. Where a plugin declares it can adjudicate the claim and does -- by returning both values, or by returning one while strictly filtering on the other -- the claim is a fact and is stored system-owned. Where a plugin declares it can and does not, nothing is stored: owning it instead would leave the sweep below able to promote, by counting users who all read the same broker security master, exactly what an authority declined to confirm. Where no enabled plugin declares knowledge of the claim at all, nothing can settle it, so it is stored owned by the user who supplied it. And where acting on the claim would have to change something the instance already holds as a fact, it is refused: nothing is stored, and the transaction resolves to the instrument the highest-precedence identifier reached.

Which of the four applies is a property of the enabled plugin set rather than of the row, so the periodic re-identification asks again. A claim stored because nothing could adjudicate it is settled or deleted once something can, and deleting one moves the postings that resolved through it.

Broker-scoped identifiers and broker descriptions are the usual case of a claim nothing can adjudicate, because no provider can be asked what a broker's own contract number or description string means. They are not a special case in the rule. A global identifier is owned in exactly the same way on an instance whose enabled plugins declare nothing that would adjudicate it, and is a fact on an instance that has one.

`instrument_identifiers` and `instrument_listing_identifiers` each carry an **owner**. Null means system-owned -- a fact in the sense above -- which is what an identifier plugin writes. A claim written owned by a user resolves for that user alone: lookups are owner-scoped with a system fallback, at both grains.
...
See adr/0062-a-user-mediated-claim-is-not-a-write-to-shared-data.md, adr/0063-identity-claims-are-owned-until-users-corroborate-them.md and adr/0065-a-plugin-declares-what-it-claims-a-call-records-what-it-claimed.md.
...
2. **Check what each key resolved to**  
   `identification_errors` says what the uploader was told, which is three
   outcomes and deliberately not which stage gave up. The stage is in telemetry,
   keyed to the run:
   ```sql
   SELECT run_started_at, key_source, key_description, candidate_outcome, outcome
   FROM telemetry.v_resolution_key
   WHERE run_job_id = '<job id>'
   ORDER BY key_description;
   ```
   `candidate_outcome = 'nothing_proposed'` is no candidate plugin returning
   hints (e.g. OpenAI failing). `outcome` separates the rest:
   `broker_description_only` is nobody recognising what was tried,
   `proposal_unconfirmed` is a result nothing corroborated, and
   `plugin_timeout` / `plugin_unavailable` are a provider not answering.
