 ...
 * **Reference Data** covers much of the shared data on the system used by all users (eg. instrument data).  Admin users are concerned with the health of the data.
    - **Instruments** are the shared instrument, listing and identifier data.  A listing is one currency a security trades in, and it is the grain a holding, a price and a dividend belong to; the security above it is what a user filtering on a company means.  The admin user is primarily concerned with the health of the data: whether there are large numbers of unidentified instruments, whether holdings are sitting on no currency line at all, or whether a large number of users are overriding the identity of a given instrument.  A holding on no line is unpriced and unvaluable, and the two ways one arises are repaired in different places -- no posting said which line, against the security holding no line for a posting to name -- so the surface reporting them counts them apart.  

    Every surface says which grain it is showing.  A line is disclosed by its currency -- "VOD (GBP)" -- which tells a user that two lines of one security are an FX rate apart in a way a MIC does not.  Holdings, checkpoints and prices show the line; instrument names and portfolio filters stay at the security, because a user filtering on a company means the company; an identifier is shown under the grain it names, since a ticker names a line and an ISIN names the security.  A column that can answer only one of the two says which it answers: the transaction list shows what the figures on a row are denominated in, which is the security's currency where the source named one and the account's where it did not.
