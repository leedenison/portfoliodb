...
func seedBalancedGroups(t testing.TB, p *Postgres, groups int) (string, string) {
	...
	instID, _, err := p.EnsureInstrument(ctx, "STOCK", "USD", "BENCH", "", "", []db.IdentifierInput{{
	...
	if err := p.ReplaceTxsInPeriod(ctx, userID, "BENCH", "", from, before, txs, resolvedFor(t, p, ids), ws, nil); err != nil {
...
func seedPairedJournals(t testing.TB, p *Postgres, groups int) (string, string, string) {
	...
	mergedAway, _, err := p.EnsureInstrument(ctx, "STOCK", "USD", "MRGA", "", "", []db.IdentifierInput{{
	...
	survivor, _, err := p.EnsureInstrument(ctx, "STOCK", "USD", "MRGB", "", "", []db.IdentifierInput{{
	...
	from := timestamppb.New(base.Add(-time.Hour))
	before := timestamppb.New(base.Add(time.Duration(groups+60) * time.Minute))
	if err := p.ReplaceTxsInPeriod(ctx, userID, "MERGE", "", from, before, txs, resolvedFor(t, p, ids), ws, nil); err != nil {
