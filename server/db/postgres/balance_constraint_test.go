...
func TestTxGroupBalance_SplitRecomputeIsNotAffected(t *testing.T) {
	...
	if err := p.ReplaceTxsInPeriod(ctx, userID, "IBKR", "", from, to, txs, resolvedFor(t, p, []string{instID, usd}), ws, nil); err != nil {
...
// delete part of a group at all. Deleting one leg of a balanced group is a violation on
// its own; the routed counterparty that negates what is left is written in the same
