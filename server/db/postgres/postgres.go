...
func (p *Postgres) GetUserByAuthSub(ctx context.Context, authSub string) (string, error) {
	var id uuid.UUID
	err := p.QueryRowContext(ctx, `SELECT id FROM users WHERE auth_sub = $1`, authSub).Scan(&id)
...
func (p *Postgres) DeletePortfolio(ctx context.Context, portfolioID string) error {
	...
	res, err := p.ExecContext(ctx, `DELETE FROM portfolios WHERE id = $1`, portUUID)
	if err != nil {
...
func (p *Postgres) PortfolioBelongsToUser(ctx context.Context, portfolioID, userID string) (bool, error) {
	...
	var count int
	err = p.QueryRowContext(ctx, `SELECT 1 FROM portfolios WHERE id = $1 AND user_id = $2`, portUUID, userUUID).Scan(&count)
	if err == sql.ErrNoRows {
		return false, nil
	}
	if err != nil {
...
func (p *Postgres) GetJob(ctx context.Context, jobID string) (apiv1.JobStatus, []*apiv1.ValidationError, string, error) {
	...
	err = p.QueryRowContext(ctx, `SELECT status, portfolio_id FROM ingestion_jobs WHERE id = $1`, jobUUID).Scan(&statusStr, &portfolioID)
	if err == sql.ErrNoRows {
	...
	rows, err := p.QueryContext(ctx, `SELECT row_index, field, message FROM validation_errors WHERE job_id = $1 ORDER BY row_index`, jobUUID)
	if err != nil {
...
func (p *Postgres) SetJobStatus(ctx context.Context, jobID string, status apiv1.JobStatus) error {
	...
	_, err = p.ExecContext(ctx, `UPDATE ingestion_jobs SET status = $2 WHERE id = $1`, jobUUID, jobStatusToStr(status))
	if err != nil {
...
func (p *Postgres) AppendValidationErrors(ctx context.Context, jobID string, errs []*apiv1.ValidationError) error {
	...
	for _, e := range errs {
		_, err = p.ExecContext(ctx, `INSERT INTO validation_errors (job_id, row_index, field, message) VALUES ($1, $2, $3, $4)`,
...
func (p *Postgres) ListPendingJobIDs(ctx context.Context) ([]string, error) {
	rows, err := p.QueryContext(ctx, `SELECT id FROM ingestion_jobs WHERE status = 'PENDING' ORDER BY created_at`)
	if err != nil {
