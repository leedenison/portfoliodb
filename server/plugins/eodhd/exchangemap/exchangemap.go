// Package exchangemap provides bidirectional EODHD exchange code <-> ISO MIC lookups.
...
// ExchangeMap provides bidirectional EODHD code <-> MIC lookups.
...
// MICToEODHDCode returns the EODHD exchange code for an ISO 10383 MIC.
func (m *ExchangeMap) MICToEODHDCode(mic string) (string, bool) {
	code, ok := m.micToCode[mic]
	return code, ok
}

// EODHDCodeToMICs returns the operating MIC(s) for an EODHD exchange code.
func (m *ExchangeMap) EODHDCodeToMICs(code string) []string {
