...
// Config is the plugin config JSON every Massive plugin reads. The three plugins
...
type Config struct {
	...
	BaseURL     string `json:"massive_base_url"` // for testing
	CallsPerMin *int   `json:"massive_calls_per_min"`

	// Days before an option's expiry stops being worth an API call. Only the
	// identifier plugin reads it, and omitempty keeps it out of the other two
	// plugins' default config rather than offering them a knob that does
	// nothing.
	ExpiredDerivativeHorizon *int `json:"expired_derivative_horizon,omitempty"`
}
