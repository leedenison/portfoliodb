...
//
// Grouping travels as a group_ref numbered within the window rather than as the
// stored group id, which is generated by this instance and means nothing to
// another one. The field is transitional: the server is becoming the thing that
// decides which postings are legs of one event, and it goes when it does. See
...
func txWindows(rows []db.ExportPosting, periodFrom, periodBefore *timestamppb.Timestamp) []*archivev1.TxWindow {
	...
	for _, r := range rows {
		...
		if groups == 0 || r.GroupID != curGroupID {
			curGroupID = r.GroupID
			groupRef = fmt.Sprintf("g%d", groups)
			groups++
		}
		last = r.Timestamp
		p := posting(r)
		p.GroupRef = proto.String(groupRef)
