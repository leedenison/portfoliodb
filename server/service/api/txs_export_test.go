...
// The legs of one group come out together and in order, which is worth nothing to
// the importer -- it derives the partition from the evidence, not from the order --
// and a good deal to whoever reads the file.
func TestTxWindows_KeepsAGroupsLegsTogether(t *testing.T) {
	rows := []dbpkg.ExportPosting{
	...
	// Flat, in the order the scan returned them. Nothing on a posting says which
