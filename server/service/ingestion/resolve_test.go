...
func TestResolve_AllPluginsErrNotIdentified_BrokerDescriptionOnly(t *testing.T) {
	...
	source := "IBKR:test:statement"
	database.EXPECT().
		FindInstrumentWithMetaByIdentifier(gomock.Any(), gomock.Any(), "MIC_TICKER", "", "UNKNOWN").
		Return("", "", nil, nil)
	database.EXPECT().
		FindInstrumentByTypeAndValue(gomock.Any(), gomock.Any(), "MIC_TICKER", "UNKNOWN").
		Return("", nil)
...
func TestResolve_OnePluginSuccess_EnsureInstrumentWithResult(t *testing.T) {
	...
	database.EXPECT().EnsureInstrument(
		gomock.Any(), gomock.Any(), "STOCK", "USD", "Apple Inc.", gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), "", nil, gomock.Any()).
...
func pathATestDB(t *testing.T, ctrl *gomock.Controller) (*mock.MockDB, *identifier.Registry) {
	...
	database.EXPECT().LookupOperatingMIC(gomock.Any(), gomock.Any()).DoAndReturn(func(_ context.Context, mic string) (string, error) { return mic, nil }).AnyTimes()
	database.EXPECT().SaveProviderIdentifiers(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(nil).AnyTimes()
	database.EXPECT().FindInstrumentWithMetaByIdentifier(gomock.Any(), gomock.Any(), "ISIN", "", "US0378331005").Return("", "", nil, nil)
...
func TestResolve_PluginReturnsUnderlying_ResolvesUnderlyingThenDerivative(t *testing.T) {
	...
	database.EXPECT().
		FindInstrumentWithMetaByIdentifier(gomock.Any(), gomock.Any(), "MIC_TICKER", "", "AAPL").
		Return("underlying-uuid", "", nil, nil)
...
func TestResolve_TwoPlugins_HigherPrecedenceWins(t *testing.T) {
	...
	ctx := context.Background()
	database.EXPECT().
		FindInstrumentWithMetaByIdentifier(gomock.Any(), gomock.Any(), "MIC_TICKER", "", "X").
		Return("", "", nil, nil)
	database.EXPECT().
		FindInstrumentByTypeAndValue(gomock.Any(), gomock.Any(), "MIC_TICKER", "X").
		Return("", nil)
...
func TestResolve_TwoPlugins_MergedIdentifiersByPrecedence(t *testing.T) {
	...
	ctx := context.Background()
	database.EXPECT().
		FindInstrumentWithMetaByIdentifier(gomock.Any(), gomock.Any(), "MIC_TICKER", "", "Y").
		Return("", "", nil, nil)
	database.EXPECT().
		FindInstrumentByTypeAndValue(gomock.Any(), gomock.Any(), "MIC_TICKER", "Y").
		Return("", nil)
...
func TestResolve_TwoPlugins_SameType_HighPrecedenceWins(t *testing.T) {
	...
	ctx := context.Background()
	database.EXPECT().
		FindInstrumentWithMetaByIdentifier(gomock.Any(), gomock.Any(), "MIC_TICKER", "", "Z").
		Return("", "", nil, nil)
	database.EXPECT().
		FindInstrumentByTypeAndValue(gomock.Any(), gomock.Any(), "MIC_TICKER", "Z").
		Return("", nil)
...
// transaction, whose source named no class, is routed to security plugins but
// not to cash plugins.
func TestRunDescriptionPluginsBatch_TransferSkipsCash(t *testing.T) {
	...
	// A TRANSFER row: its source named no class, so HintsFromTx floors it at
	// SECURITY, which is what keeps it away from the cash plugin.
	items := []candpkg.BatchItem{
		{ID: "t-1", InstrumentDescription: "ABNB", Hints: identifier.Hints{
			SecurityTypeHint: identifier.SecurityTypeHintSecurity,
			Currency:         "USD",
		}},
	}
...
func TestResolve_ConflictingHintsDegradeRatherThanFailingTheRow(t *testing.T) {
	...
	database.EXPECT().EnsureInstrument(gomock.Any(), gomock.Any(), "", "", "", "", "",
...
func TestResolve_PluginFailsThenRetrySucceeds(t *testing.T) {
	...
	ctx := context.Background()
	database.EXPECT().
		FindInstrumentWithMetaByIdentifier(gomock.Any(), gomock.Any(), "MIC_TICKER", "", "RETRY").
		Return("", "", nil, nil)
	database.EXPECT().
		FindInstrumentByTypeAndValue(gomock.Any(), gomock.Any(), "MIC_TICKER", "RETRY").
		Return("", nil)
