...
pub async fn populate_database(db_manager: &DatabaseManager, test_db: &TestDatabase) -> Result<()> {
    ...
    for (instrument, identifiers, derivative) in &test_db.instruments {
        ...
        if instrument.dbid == 0 {
            return Err(anyhow::anyhow!(
                "Explicit dbid required for instrument with dbid 0"
            ));
        }
        ...
        for identifier in identifiers {
            ...
            if identifier.dbid == 0 {
                return Err(anyhow::anyhow!(
                    "Explicit dbid required for identifier: {}",
                    identifier.id
                ));
            }
...
pub async fn verify_database(db_manager: &DatabaseManager, expected: &TestDatabase) -> Result<()> {
    ...
    let expected_instrument_dbids: Vec<i64> = expected
        .instruments
        .iter()
        .map(|(instrument, _, _)| instrument.dbid)
        .collect();
    let actual_instrument_dbids: Vec<i64> = actual_instruments
        .iter()
        .map(|instrument| instrument.dbid)
        .collect();

    verify_dbids_exist(
        &expected_instrument_dbids,
        &actual_instrument_dbids,
        "instrument",
    );

    // Verify identifiers
    let actual_identifiers = Identifiers::find().all(conn).await?;
    let expected_identifier_count: usize = expected
        .instruments
        .iter()
    ...
    let expected_identifier_dbids: Vec<i64> = expected
        .instruments
        .iter()
        .flat_map(|(_, identifiers, _)| identifiers.iter().map(|identifier| identifier.dbid))
        .collect();
    let actual_identifier_dbids: Vec<i64> = actual_identifiers
        .iter()
        .map(|identifier| identifier.dbid)
        .collect();

    verify_dbids_exist(
        &expected_identifier_dbids,
        &actual_identifier_dbids,
        "identifier",
    );

    // Verify derivatives
    let actual_derivatives = Derivatives::find().all(conn).await?;
    let expected_derivative_count = expected
        .instruments
        .iter()
    ...
    let expected_derivative_dbids: Vec<i64> = expected
        .instruments
        .iter()
        .filter_map(|(_, _, derivative)| derivative.as_ref().map(|d| d.dbid))
        .collect();
    let actual_derivative_dbids: Vec<i64> = actual_derivatives
        .iter()
        .map(|derivative| derivative.dbid)
        .collect();

    verify_dbids_exist(
        &expected_derivative_dbids,
        &actual_derivative_dbids,
        "derivative",
    );
    ...
    let expected_transaction_dbids: Vec<i64> = expected
        .transactions
        .iter()
        .map(|transaction| transaction.dbid)
        .collect();
    let actual_transaction_dbids: Vec<i64> = actual_transactions
        .iter()
        .map(|transaction| transaction.dbid)
        .collect();

    verify_dbids_exist(
        &expected_transaction_dbids,
        &actual_transaction_dbids,
        "transaction",
    );
    ...
    let expected_price_dbids: Vec<i64> = expected.prices.iter().map(|price| price.dbid).collect();
    let actual_price_dbids: Vec<i64> = actual_prices.iter().map(|price| price.dbid).collect();
